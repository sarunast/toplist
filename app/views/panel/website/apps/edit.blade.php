...
<div class="wmdEditDiv" style="display: none">
    <form action="{{url('panel/website/content/edit')}}" id="LoginForm" method="post" >
        <input name="title" class="form-control" minlength="5" maxlength="100" value="{{{$pagePost->title}}}" required placeholder="Title">
        <div id="raptor-docked-element">{{$pagePost->content}}</div>
        <input type="hidden" name="content">
        <p class="text-info" id="errorMessage"  style="display: none"><em>Content must have at least 5 characters</em></p>
        <input type="hidden" name="_token" value="{{ csrf_token() }}">
        <input type="hidden" name="content_id" value="{{{$pagePost->id}}}">
        <input type="submit" id="sendcontent" Value="Save Changes" class="btn btn-info btn-sm">


    </form>
</div>
<form method="post" class="text-right" style="margin-top: -30px" action="{{url('panel/website/content/'.$pagePost->id)}}">
    <input type="hidden" name="_token" value="{{ csrf_token() }}">
