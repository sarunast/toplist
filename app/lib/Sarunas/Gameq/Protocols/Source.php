...
class Source extends Protocols {
	...
	protected function _process_challenge($packets) {
		$buf = new \Sarunas\Gameq\Buffer($packets[0]);
		$head = $buf->read(4);
	...
	protected function _preparePackets($packets) {
		$buffer = new \Sarunas\Gameq\Buffer($packets[0]);
		...
		foreach($packets as $packet) {
			...
			$buffer = new \Sarunas\Gameq\Buffer($packet);
	...
	protected function _process_players($packets) {
		...
		// Create a new buffer
		$buf = new \Sarunas\Gameq\Buffer($packet);
	...
	protected function _process_details($packets) {
		...
		$buf = new \Sarunas\Gameq\Buffer($data);
