...
class Unreal2 extends \Gameq\Protocols {
	...
	protected function _readBadPascalString(\Gameq\Buffer &$buf) {
		$len = $buf->readInt8();
	...
	protected function _readColoredPascalString(\Gameq\Buffer &$buf) {
		$len = $buf->readInt8();
		$str = "";
	...
	*/
	...
	protected function _readUnrealString(\Gameq\Buffer &$buf) {
	...
	protected function _process_status($packets) {
		$buf = new \Gameq\Buffer($this->_preparePackets($packets));
